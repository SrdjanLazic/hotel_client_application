...
public class NotificationServiceRestClient {
    ...
    public static final String URL = "http://localhost:8084/hotel-notification-service";
    ...
    public NotificationListDto getManagerNotifications(Long id) throws IOException {
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        JavaTimeModule module = new JavaTimeModule();
        objectMapper.registerModule(module);
        objectMapper.configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, false);
        ...
        Request request = new Request.Builder()
                .url(URL + String.format("/notification/manager/%d", id))
                .header("Authorization", "Bearer " + HotelClientApplication.getInstance().getToken())
                .get()
                .build();
